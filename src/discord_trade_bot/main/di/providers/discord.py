...
from discord_trade_bot.infrastructure.taskiq.tasks import process_signal_task, update_signal_task
...
class DiscordProvider(Provider):
    ...
    async def get_discord_client(self, config: AppConfig) -> AsyncIterable[DiscordSelfAdapter]:
        ...
        async def on_message_edit_wrapper(dto: ProcessSignalDTO):
            # Send task to Redis queue (pass as separate parameters for MSGPack serialization)
            await update_signal_task.kiq(source_id=dto.source_id, channel_id=dto.channel_id, message_id=dto.message_id, text=dto.text)

        client = DiscordSelfAdapter(
            token=token,
            on_message_callback=on_message_wrapper,
            on_message_edit_callback=on_message_edit_wrapper,
